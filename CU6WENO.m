function [FP1,FM1] = CU6WENO(Q,g)
%%% Uses WENO6 to compute F_j+1/2 at each grid point
%-- Also need to account for F_j-1/2

% Utilizes Roe wave decomposition (efficient since already doing
% characteristic mapping) to deteremine appropriate stencil set
...
for ii=1:nx
  ...
  for ll=1:neqs
    ...
    if LAMS(ll)>=0
      VLX(ll) = WENO6RIGHT(gvec,ii);
    elseif LAMS(ll)<0
      VLX(ll) = WENO6LEFT(gvec,ii);
    end
  end
...
%%% TEST PROBLEM HARD BC FIX (kills waves)
FM1(1:4,:) = FP1(1:4,:);
