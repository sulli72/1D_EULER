function [FP1,FM1] = XWENO_6_LF_CHAR(Q,g)
%%% Uses WENO6 to compute F_j+1/2 at each grid point
%-- Also need to account for F_j-1/2

% Same philosophy as WENO_5_LF_CHAR routine, but now using different
% stencils, weights, and smoothness indicators to obtain 6th order accurate
% solution in smooth regions of flow
...
for ii=1:nx
  ...
  % Used for local Roe type flux splitting
%   alpha1=LAMS(1);
%   alpha2=LAMS(2);
%   alpha3=LAMS(3);
  ...
  for ll=1:neqs
    POSIN=VPLS(ll,:);
    VPOSRECON(ll) = WENO6RIGHT(POSIN,ii);

    NEGIN=VMNS(ll,:);
    VMINRECON(ll) = WENO6LEFT(NEGIN,ii);
  end
